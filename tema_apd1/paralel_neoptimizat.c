...
int main( int argc, char *argv[] )
{
	...
	for(p = 0; p < nr_weeks; p++)
	{
		...
		/* Parcurgem unul cate unul toti senatorii. Deoarece transformarile efectuate in
		aceasta saptamana asupra unui senator x nu afecteaza un alt senator y
		aceasta portiune poate fi usor paralelizata. */
		#pragma omp parallel for \
		shared(configuration, a, b, N, num_colours) \
		private(i, j, max_loc, max_val, l, k, aux, maxX, maxY, c)

		for(i = 0; i < N; i++)
			for(j = 0; j < N; j++)
			{
				/* Initializare vector distante culori. */
				...
				/* Actualizam vectorul de contorizare a culorilor senatorilor, avand grija
				ca 2 thread-uri sa nu scrie in acelasi timp. */
